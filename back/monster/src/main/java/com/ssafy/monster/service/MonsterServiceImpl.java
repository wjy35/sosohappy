    ...
    */
    ...
    public List<MonsterRes> searchMonsterList(Long memberId){
        ...
        List<MonsterRes> resList = monsterList
                .stream().map(m -> MonsterMapper.INSTANCE.toLMonsterRes(m, getCurrentPoint(m.getMonsterClover()).first(), getCurrentPoint(m.getMonsterClover()).second())
    ...
    public MonsterRes updateMonsterClover(Long memberMonsterId, int clover) {
        ...
        //result
        MonsterRes monsterRes = MonsterMapper.INSTANCE.toLMonsterRes(growth, pair.first(), pair.second());
